...
Every cert-scoped page renders inside this tree:
```
TopNav (fixed, z-100, height 56)
  ├─ Logo → /dashboard (logged in) or / (logged out)
  ├─ Cert badge → cert switcher dropdown
  ├─ Nav tabs (Study/Exam/History) — only on cert pages
  ├─ Theme toggle (dark/light)
  └─ User avatar menu (Dashboard / Admin / Sign out)
...
```
`TopNav` renders itself inside each page. `AppShell` wraps page content. `TutorChat` is rendered **only** by `AppShell` — pages never import it.

## Auth lifecycle

1. User clicks Google/GitHub/Email on `/login` → redirects to `${VITE_WORKER_URL}/auth/{provider}/login`
2. Worker authenticates, signs JWT with HMAC-SHA256 (`JWT_SECRET`), redirects back to `/login?token=...`
3. `LoginPage` extracts token from URL, parses JWT payload, calls `authStore.setAuth(user, token)`
4. Token stored in `localStorage` as `ccxp_jwt`
5. `authStore.init()` reads token from `localStorage` on app startup, sets `user` + `token` state
6. `ProtectedRoute` gates routes — redirects to `/login` if no `user`
7. Admin access: worker compares JWT email to `ADMIN_EMAIL` secret, frontend reads `user.isAdmin`
8. Logout clears `ccxp_jwt` from `localStorage` and resets store state
...
ExamPage navigates to /exam/loading
  ↓
LoadingScreen → useQuestionGen hook → POST ${VITE_WORKER_URL}/api/llm (type: generate-questions)
  ↓
LLM returns JSON → questionBank.save() to localStorage → examStore.setQuestions()
  ↓
ExamPage navigates to /exam/question → QuestionCard renders one at a time
  ↓
User answers → examStore.answerQuestion(id, answer)
  ↓
Submit (or timer expire) → examStore.submitExam() → navigate to /:certId/results
  ↓
ResultsPage reads questions/answers from examStore, tracks via POST /api/admin/track
  ↓
HistoryPage reads from historyStore (persisted to localStorage as ${userId}_exam_history)
```
- `setMode()` clears all prior answers/questions — always called before generating new set
- Retake flow skips LLM: reads from `questionBank` directly, shuffles with Fisher-Yates
